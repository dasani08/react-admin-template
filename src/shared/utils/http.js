...
class Http {
  ...
  api(
    ...
    method = 'get',
    params = {},
    host = process.env.REACT_APP_API_BASE_URL || 'http://localhost:3000'
  ) {
    return new window.Promise((resolve, reject) => {
      ...
      this.request[method](path, params).then(resolve).catch(reject);
    });
  }
  ...
  put(path, data = {}) {
    return this.api(path, 'put', data).then((res) => res.data);
  }
